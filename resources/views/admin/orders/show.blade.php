...
@section('title', 'Detail Pesanan '.$order->code.' (Admin)')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h1 class="h4 mb-1">Pesanan {{ $order->code }}</h1>
            <p class="text-muted small mb-0">Perbarui status dan lihat detail item pesanan.</p>
        </div>
    ...
    <div class="row g-4">
        <div class="col-lg-7">
            <div class="card shadow-sm border-0 mb-3">
                <div class="card-body">
                    <h5 class="card-title mb-3">Pelanggan & Pengiriman</h5>
                    <p class="mb-1"><span class="text-muted">Pelanggan:</span> {{ $order->user->name ?? '-' }} ({{ $order->user->email ?? '-' }})</p>
                    <p class="mb-1"><span class="text-muted">Nama penerima:</span> {{ $order->recipient_name }}</p>
                    <p class="mb-1"><span class="text-muted">Alamat:</span> {{ $order->address }}, {{ $order->city }}, {{ $order->province }} ({{ $order->postal_code }})</p>
                    <p class="mb-0"><span class="text-muted">Kurir:</span> {{ strtoupper($order->courier) }} - {{ $order->service }}</p>
                </div>
            </div>
        ...
        <div class="col-lg-5">
            <div class="card shadow-sm border-0 mb-3">
                <div class="card-body">
                    <h6 class="fw-semibold mb-3">Kirim Pesan ke Pelanggan</h6>
                    <form method="POST" action="{{ route('admin.orders.message', $order) }}" class="row g-3">
                        @csrf
                        <div class="col-12">
                            <textarea name="body" rows="3" class="form-control" placeholder="Tulis pesan untuk pelanggan..." required></textarea>
                        </div>
                        <div class="col-12">
            ...
            <div class="card shadow-sm border-0 mb-3">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.orders.update-status', $order) }}" method="POST" class="row gy-2">
                        @csrf
                        @method('PATCH')
                        <div class="col-12">
